...
**Pós-MVP (foco atual):** a **Fase 003C / B012** introduz a arquitetura de **perfis profissionais do portal** (`PerfilProfissionalPortal`, ADR-014): o `ECNHClient` deixa de assumir apenas Psicólogo e passa a resolver o perfil pelo HTML (e opcionalmente por config), permitindo Médico e **perfis futuros** com mínimo impacto em parser, repositório, serviços e jobs.
...
Dois comportamentos reais do portal ainda **não são automatizados** e podem impedir a sincronização de alguns profissionais. Ambos devem ser **reavaliados após a validação completa da arquitetura de perfis (B012)** — a nova arquitetura poderá absorver parcial ou totalmente essas necessidades:
...
Detalhes: [COMPORTAMENTOS_PORTAL_HOMOLOGACAO.md](COMPORTAMENTOS_PORTAL_HOMOLOGACAO.md). Fluxo feliz: [API.md](API.md) / [FLUXO_HTTP.md](FLUXO_HTTP.md). Foco arquitetural atual: [BACKLOG.md](BACKLOG.md) — **B012**.
