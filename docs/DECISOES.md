...
- **Evolução (03/08/2026):** reordenação visual — `PROFISSIONAL` passa a ficar imediatamente após `EMAIL`; `Tipo de Processo` e `Categoria` vêm em seguida. O layout 10 anterior permanece como variante de leitura (`CABECALHOS_ABA_AGENDA_OFICIAL_V10_TIPO_ANTES_PROFISSIONAL`) para migração automática.
